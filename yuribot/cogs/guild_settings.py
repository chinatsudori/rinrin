...
class GuildSettings(commands.Cog):
    ...
    @app_commands.command(
        name="set_channel", description="Set a per-guild channel setting by key."
    )
    @app_commands.describe(
        key="e.g., log_channel, modlog_channel, welcome_channel",
    ...
    async def set_channel(
        ...
        await interaction.response.send_message(
            f"Set `{key}` to {channel.mention}", ephemeral=True
        )

    @app_commands.command(
        name="get_setting", description="Get a per-guild setting by key."
    )
    ...
    @app_commands.command(
        name="list_settings", description="List all per-guild settings."
    )
    @app_commands.checks.has_permissions(manage_guild=True)
    async def list_settings(self, interaction: discord.Interaction):
        ...
        if not rows:
            return await interaction.response.send_message(
                "No settings found for this guild.", ephemeral=True
            )
