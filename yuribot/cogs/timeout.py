...
class TimeoutCog(commands.Cog):
    ...
    @app_commands.command(
        name="timeout",
        description="Timeout a member for custom days/hours/minutes (Discord cap ~28 days).",
    )
    ...
    async def timeout(
        ...
        if delta.total_seconds() < 60:
            return await interaction.followup.send(S("timeout.error.min_duration"), ephemeral=True)
        ...
        if dm_user:
            try:
                ...
                embed.add_field(
                    ...
                    value=S("timeout.dm.value.duration", d=days, h=hours, m=minutes),
                    inline=True,
                )
                embed.add_field(
        ...
        try:
            ...
            return await interaction.followup.send(S("timeout.error.forbidden_apply"), ephemeral=True)
        except discord.HTTPException as e:
            return await interaction.followup.send(S("timeout.error.http_apply", err=e), ephemeral=True)
    ...
    @app_commands.describe(user="Member to remove timeout from", reason="Optional reason")
    async def untimeout(self, interaction: discord.Interaction, user: discord.Member, reason: str | None = None):
        ...
        try:
            ...
            return await interaction.followup.send(S("timeout.error.forbidden_remove"), ephemeral=True)
        except discord.HTTPException as e:
            return await interaction.followup.send(S("timeout.error.http_remove", err=e), ephemeral=True)
        ...
        await interaction.followup.send(S("timeout.remove.done", user=user.mention), ephemeral=True)
