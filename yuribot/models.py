...
def upsert_guild_cfg(guild_id:int, ann:int, planning_forum:int, polls:int, discussion_forum:int|None=None) -> None:
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
...
def get_guild_cfg(guild_id:int):
    with connect() as con:
        cur = con.cursor()
        row = cur.execute(
            "SELECT announcements_channel_id, planning_forum_id, polls_channel_id, discussion_forum_id FROM guild_config WHERE guild_id=?",
            (guild_id,)
        ).fetchone()
...
def upsert_club_cfg(guild_id:int, club_type:str,
                    ann:int, planning:int, polls:int, discussion:int) -> int:
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
        ...
        con.commit()
        row = cur.execute("SELECT id FROM clubs WHERE guild_id=? AND club_type=?", (guild_id, club_type)).fetchone()
        return row[0]

def get_club_cfg(guild_id:int, club_type:str):
    with connect() as con:
        cur = con.cursor()
        row = cur.execute("""
            SELECT id, announcements_channel_id, planning_forum_id, polls_channel_id, discussion_forum_id
            FROM clubs WHERE guild_id=? AND club_type=?""", (guild_id, club_type)).fetchone()
    if not row: return None
    return {
        "club_id": row[0],
...
def get_club_by_planning_forum(guild_id:int, forum_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""
            SELECT id, club_type FROM clubs
            WHERE guild_id=? AND planning_forum_id=?""", (guild_id, forum_id)).fetchone()
...
def open_collection(guild_id:int, club_id:int, opens_at:str, closes_at:str) -> int:
    with connect() as con:
        cur = con.cursor()
        cur.execute("INSERT INTO collections (guild_id, club_id, opens_at, closes_at, status) VALUES (?, ?, ?, ?, 'open')",
                    (guild_id, club_id, opens_at, closes_at))
        con.commit()
        return cur.lastrowid

def latest_collection(guild_id:int, club_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""
        ...
        """, (guild_id, club_id)).fetchone()

def close_collection_by_id(collection_id:int):
    with connect() as con:
        cur = con.cursor()
        cur.execute("UPDATE collections SET status='closed' WHERE id=?", (collection_id,))
        con.commit()

def add_submission(guild_id:int, club_id:int, collection_id:int, author_id:int, title:str, link:str, thread_id:int, created_at:str) -> int:
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
...
def list_submissions_for_collection(collection_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""
...
def get_submission(collection_id:int, ordinal:int):
...
def get_submissions_by_ordinals(collection_id:int, ordinals:List[int]) -> list[tuple]:
    ...
    for o in ordinals:
        if 1 <= o <= len(rows) and o not in seen:
            out.append(rows[o-1]); seen.add(o)
    return out

def create_poll(guild_id:int, club_id:int, channel_id:int, created_at:str, closes_at:str|None) -> int:
    with connect() as con:
        cur = con.cursor()
        cur.execute("""INSERT INTO polls (guild_id, club_id, channel_id, created_at, closes_at, status)
                       VALUES (?, ?, ?, ?, ?, 'open')""",
                    (guild_id, club_id, channel_id, created_at, closes_at))
        con.commit()
        return cur.lastrowid

def add_poll_option(poll_id:int, label:str, submission_id:int):
    with connect() as con:
        cur = con.cursor()
        cur.execute("INSERT INTO poll_options (poll_id, label, submission_id) VALUES (?, ?, ?)", (poll_id, label, submission_id))
        con.commit()

def set_poll_message(poll_id:int, channel_id:int, message_id:int):
    with connect() as con:
        cur = con.cursor()
...
def get_poll_channel_and_message(poll_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("SELECT channel_id, message_id, guild_id FROM polls WHERE id=?", (poll_id,)).fetchone()

def create_series(guild_id:int, club_id:int, title:str, link:str, source_submission_id:int|None) -> int:
    with connect() as con:
        cur = con.cursor()
        cur.execute("""INSERT INTO series (guild_id, club_id, title, link, source_submission_id, status)
                       VALUES (?, ?, ?, ?, ?, 'active')""",
                    (guild_id, club_id, title, link, source_submission_id))
        con.commit()
        return cur.lastrowid

def latest_active_series(guild_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("SELECT id, title, link FROM series WHERE guild_id=? AND status='active' ORDER BY id DESC LIMIT 1",
                           (guild_id,)).fetchone()

def add_discussion_section(series_id:int, label:str, start_ch:int, end_ch:int, start_iso:str, event_id:int|None):
    with connect() as con:
        cur = con.cursor()
        cur.execute("INSERT INTO schedule_sections (series_id, label, start_chapter, end_chapter, discussion_event_id, discussion_start) VALUES (?, ?, ?, ?, ?, ?)",
                    (series_id, label, start_ch, end_ch, event_id, start_iso))
        con.commit()
def list_series(guild_id:int, club_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""SELECT id, title, link, status
                              FROM series WHERE guild_id=? AND club_id=?
                              ORDER BY id DESC""", (guild_id, club_id)).fetchall()

def get_series(series_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("SELECT id, guild_id, title, link, status FROM series WHERE id=?", (series_id,)).fetchone()

def latest_active_series(guild_id:int, club_id:int):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""SELECT id, title, link FROM series
                              WHERE guild_id=? AND club_id=? AND status='active'
                              ORDER BY id DESC LIMIT 1""", (guild_id, club_id)).fetchone()

def due_discussions(now_iso:str, limit:int=25):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""
...
def mark_discussion_posted(section_id:int, thread_id:int):
    with connect() as con:
        cur = con.cursor()
        cur.execute("UPDATE schedule_sections SET posted=1, discussion_thread_id=? WHERE id=?", (thread_id, section_id))
        con.commit()
...
def get_mod_logs_channel(guild_id: int) -> int | None:
    with connect() as con:
        cur = con.cursor()
        row = cur.execute("SELECT mod_logs_channel_id FROM guild_settings WHERE guild_id=?", (guild_id,)).fetchone()
        return row[0] if row else None


def add_mod_action(guild_id: int, target_user_id: int, target_username: str,
                   rule: str, offense: int, action: str, details: str | None,
                   evidence_url: str | None, actor_user_id: int, created_at: str) -> int:
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (guild_id, target_user_id, target_username, rule, offense, action, details or "", evidence_url or "", actor_user_id, created_at))
        con.commit()
        return cur.lastrowid
...
def list_mod_actions_for_user(guild_id: int, target_user_id: int, limit: int = 20) -> list[tuple]:
    with connect() as con:
        ...
        return cur.execute("""
            ...
            ORDER BY id DESC
            LIMIT ?
        """, (guild_id, target_user_id, limit)).fetchall()
def set_bot_logs_channel(guild_id: int, channel_id: int):
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
...
def get_bot_logs_channel(guild_id: int) -> int | None:
    with connect() as con:
        cur = con.cursor()
        row = cur.execute("SELECT bot_logs_channel_id FROM guild_settings WHERE guild_id=?",
                          (guild_id,)).fetchone()
        return row[0] if row else None

def set_welcome_settings(guild_id: int, channel_id: int, image_filename: str):
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
...
def get_welcome_settings(guild_id: int):
    with connect() as con:
        ...
        if not row:
            return None
        return {"welcome_channel_id": row[0], "welcome_image_filename": row[1] or "welcome.png"}

def bump_emoji_usage(guild_id: int, when_iso: str, emoji_key: str, emoji_name: str,
                     is_custom: bool, via_reaction: bool, inc: int = 1):
...
def top_emojis(guild_id: int, month: str, limit: int = 20):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""
...
def bump_sticker_usage(guild_id: int, when_iso: str, sticker_id: int, sticker_name: str, inc: int = 1):
    month = when_iso[:7]
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
...
def top_stickers(guild_id: int, month: str, limit: int = 20):
    with connect() as con:
        cur = con.cursor()
        return cur.execute("""
...
def bump_member_message(guild_id: int, user_id: int, when_iso: str | None = None, inc: int = 1):
...
def reset_member_activity(guild_id: int, scope: str = "month", month: str | None = None):
    """Admin utility. scope: 'month' (requires month) or 'all'."""
    with connect() as con:
        cur = con.cursor()
        if scope == "month" and month:
            cur.execute("DELETE FROM member_activity_monthly WHERE guild_id=? AND month=?", (guild_id, month))
        elif scope == "all":
...
def get_movie_cfg(guild_id: int):
    row = get_club_cfg(guild_id, "movie")
    return row

def set_movie_cfg(guild_id: int, announcements_id: int, projection_channel_id: int, polls_id: int):
...
def create_movie_events(guild_id: int, club_id: int, title: str, link: str | None,
                        show_date_iso: str, event_id_morning: int | None, event_id_evening: int | None):
    with connect() as con:
        cur = con.cursor()
        cur.execute("""
